...
"""Data Dir"""
C.dataset_path = "/home/im/FasterSeg/data/"
...
C.test_source = osp.join(C.dataset_path, "cityscapes_test_fine.txt")
...
C.num_train_imgs = 139 # number of training images
C.num_eval_imgs = 40 # number of validation images
...
C.train_scale_array = [0.7, 1, 1.4] # scale factors for augmentation during training
...
C.eval_width = 1024 # real image width
...
if C.mode == "teacher":
    ...
    C.load_path = "search-224x224_F12.L16_batch2-20231109-174004" # path to the searched directory
    C.load_epoch = "last" # "last" or "int" (e.g. "30"): which epoch to load from the searched architecture
    C.batch_size = 6
    C.Fch = 12 # base channel number
    C.image_height = 1024 # crop size for training
    C.image_width = 1024
    ...
    C.load_path = "search-224x224_F12.L16_batch2-20231109-174004" # path to the searched directory
    C.teacher_path = "/home/im/FasterSeg/train/" # where to load the pretrained teacher's weight
