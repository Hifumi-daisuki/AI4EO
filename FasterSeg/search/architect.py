        ...
        for idx in range(len(self.optimizers)):
            ...
            if self.latency_weight[idx] > 0:
                ...
                if len(self.model._width_mult_list) == 1:
                    r0 = 1./500; r1 = 499./500
                    latency = latency + r0 * self.model.forward_latency((3, 1024, 1024), alpha=True, beta=False, ratio=False)
                    latency = latency + r1 * self.model.forward_latency((3, 1024, 1024), alpha=False, beta=True, ratio=False)
                else:
                    r0 = 1./500; r1 = 497./500; r2 = 2./500
                    latency = latency + r0 * self.model.forward_latency((3, 1024, 1024), alpha=True, beta=False, ratio=False)
                    latency = latency + r1 * self.model.forward_latency((3, 1024, 1024), alpha=False, beta=True, ratio=False)
                    latency = latency + r2 * self.model.forward_latency((3, 1024, 1024), alpha=False, beta=False, ratio=True)
